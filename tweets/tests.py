...
class TweetTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user2=User.objects.create_user(username='abc2',password='testpassword2')
        Tweet.objects.create(content="my first test test",user=self.user)
        
        Tweet.objects.create(content="my first test test",user=self.user)
        Tweet.objects.create(content="my first test test",user=self.user2)
        self.currentCount=Tweet.objects.all().count()
        

    def test_tweet_created(self):
        tweet_obj=Tweet.objects.create(content="my second test tweet",user=self.user)
    ...
    def test_tweet_create_api_view(self):
        ...
        request_data={'content':'this is test tweet'}
        client=self.get_client()
